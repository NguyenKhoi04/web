...
export default function MilestoneEditModal({
  ...
  // Linking state
  const [activeTab, setActiveTab] = useState<"info" | "sprints" | "tasks">(
    "info",
  );
  ...
  async function fetchLinkedData() {
    try {
      ...
      setAvailableSprints(
        allSprints.filter(
          (s: any) => !s.milestoneId || s.milestoneId !== milestone.id,
        ),
      );
      ...
      setAvailableTasks(
        allTasks.filter(
          (t: any) => !t.milestoneId || t.milestoneId !== milestone.id,
        ),
      );
    } catch (e) {
      console.error(e);
  ...
  async function handleLink(
    ...
    try {
      ...
      await fetch(
        `/api/projects/${projectId}/milestones/${milestone.id}/link`,
        {
          method: "POST",
        ...
        <div className="flex-1 overflow-y-auto pr-2">
          ...
          {activeTab === "sprints" && (
            <div className="space-y-4">
                  ...
                  <ul className="space-y-2">
                    {sprints.map((s) => (
                      ...
                      >
                        ...
                        <button
                          onClick={() => handleLink("sprint", s.id, "remove")}
              ...
              <div>
                <h4 className="font-medium text-sm mb-2 text-gray-700">
                  Thêm Sprint vào Milestone
                </h4>
                <div className="grid grid-cols-1 gap-2">
                  {availableSprints.map((s) => (
                    <button
                      key={s.id}
                      onClick={() => handleLink("sprint", s.id, "add")}
                      disabled={linkLoading}
                  ...
                  <ul className="space-y-2">
                    {tasks.map((t) => (
                      ...
                      >
                        ...
                        <button
                          onClick={() => handleLink("task", t.id, "remove")}
                          disabled={linkLoading}
              ...
              <div>
                ...
                <div className="h-60 overflow-y-auto border rounded-lg p-2 space-y-2">
                  {availableTasks.map((t) => (
                    <button
                      key={t.id}
                      onClick={() => handleLink("task", t.id, "add")}
                      disabled={linkLoading}
