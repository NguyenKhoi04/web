                    ...
                    {!task.attachments || task.attachments.length === 0 ? (
                      ...
                      <ul className="space-y-3">
                        {task.attachments.map((a: any) => {
                          const fileUrl = a.resource?.objectKey
                            ? `/uploads/${a.resource.objectKey}`
                            : "#";
                          return (
                            <li
                              key={a.id}
                              className="flex items-center gap-4 bg-white p-3 rounded-xl border border-gray-200 hover:shadow-md transition-shadow"
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="text-sm font-medium text-gray-900 truncate">
                                  {a.resource?.name || "Unknown file"}
                                </div>
                                <div className="text-xs text-gray-500">
                                  {(a.resource?.size / 1024 || 0).toFixed(1)} KB
                                </div>
                              </div>
                              <a
                                href={fileUrl}
                      ...
                      <div className="flex flex-wrap gap-2">
                        {task.assignees?.length > 0 ? (
                          task.assignees.map((a: any) => (
                            <span
                              key={a.userId}
                              className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-white border border-gray-200 text-gray-800"
                            >
