...
import { Card, SearchIcon, Th, Td, Avatar, RoleChip, StatusChip } from './SharedUI';
...
export default function UsersPanel() {
    ...
    async function fetchUsers(signal?: AbortSignal) {
        ...
        try {
            const res = await fetch(`/api/admin/users?query=${encodeURIComponent(q)}`, { credentials: 'include', signal });
    ...
    async function toggleSuspend(user: User) {
        ...
        setRows(prev => prev.map(u => u.id === user.id ? ({ ...u, status: next }) : u));
        try {
            const res = await fetch(`/api/admin/users/${user.id}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include',
                body: JSON.stringify({ action: next === 'ACTIVE' ? 'ACTIVATE' : 'SUSPEND' }),
            });
            if (!res.ok) throw new Error(`HTTP ${res.status}`);
        } catch (e) {
            // revert
            setRows(prev => prev.map(u => u.id === user.id ? ({ ...u, status: user.status }) : u));
        }
    }
                                    ...
                                    <tr key={u.id} className="hover:bg-white/10 transition-colors">
                                        <Td>
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="text-xs text-blue-300 mt-0.5">{u.email}</div>
                                                </div>
                                            </div>
                                        </Td>
                                        <Td>
                                        ...
                                        <Td className="text-right">
                                            <div className="inline-flex gap-2">
                                                ...
                                                >
                                                    {u.status === 'ACTIVE' ? '🚫 Tạm ngưng' : '✅ Mở khóa'}
                                                </button>
                                                <button
