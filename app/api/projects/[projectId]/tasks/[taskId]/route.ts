...
export async function PATCH(
  ...
  try {
    ...
    if (data.status === undefined && before.status === "TODO") {
      const nextFollower =
        data.followerId !== undefined ? data.followerId : before.followerId;
      const nextAssigneeCount =
        data.assigneeIds !== undefined
          ? data.assigneeIds.length
          : before.assignees.length;
    ...
    const updated = await prisma.$transaction(async (tx) => {
      const u = await tx.task.update({
        ...
        data: {
          ...
          followerId:
            data.followerId === undefined ? undefined : data.followerId,
      ...
      if (data.assigneeIds) {
        ...
        if (data.assigneeIds.length) {
          ...
          const addedIds = newIds.filter(
            (id) => !oldIds.includes(id) && id !== me.user.id,
          );
      ...
      if (data.attachmentIds) {
        ...
        // Delete removed
        const toDelete = currentResIds.filter(
          (id) => !data.attachmentIds!.includes(id),
        );
        ...
        // Add new
        const toAdd = data.attachmentIds.filter(
          (id) => !currentResIds.includes(id),
        );
        if (toAdd.length > 0) {
