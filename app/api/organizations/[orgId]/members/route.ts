...
export async function POST(
    ...
    try {
        ...
        if (!inviter || (inviter.role !== "OWNER" && inviter.role !== "ADMIN")) {
            return NextResponse.json({ error: "Forbidden: You cannot invite members" }, { status: 403 });
        }
        ...
        return NextResponse.json({ error: e.message || "Failed to invite member" }, { status: 500 });
    }
}
...
export async function DELETE(
    ...
    try {
        ...
        if (isSelf) {
            ...
            if (isAdmin && (targetMem.role === "ADMIN" || targetMem.role === "OWNER")) {
