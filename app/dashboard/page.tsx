...
type TeamResponse = {
  ...
  uniqueMembers: {
    ...
    email: string;
    image: string | null;
    projects: { id: string; key: string | null }[];
  }[];
};
...
const Dashboard = () => {
  ...
  async function loadProjectsOwned() {
    try {
  ...
  async function loadProjectsJoined() {
    try {
  ...
  async function acceptInvite(inviteId: string) {
    ...
    // sau khi accept: reload lời mời & dự án tôi tham gia
    loadInvites();
    loadProjectsJoined();
  ...
  // Lúc mount lần đầu: lấy cả owned + joined để dashboard có số liệu
  useEffect(() => {
    loadProjectsOwned();
    loadProjectsJoined();
  }, []);
  ...
  async function handleDeleteProject(id: string) {
    ...
    if (res.ok) {
      await loadProjectsOwned();
      await loadProjectsJoined();
    } else {
      const { error } = await res.json().catch(() => ({}));
            ...
            {activeTab === 'invites' && (
              <div className="space-y-10">
                ...
                <section>
                  ...
                  {joinedErr ? (
                    <div className="text-red-600">{joinedErr}</div>
                  ) : joinedProjects.length === 0 ? (
                    <div className="rounded-xl border bg-white p-10 text-center text-gray-600">
                      Bạn chưa ở trong dự án nào ngoài dự án của bạn.
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <section>
                  ...
                  {invitesLoading ? (
                    ...
                    <div className="text-red-600">{invitesErr}</div>
                  ) : invites.length === 0 ? (
                    <div className="rounded-xl border bg-white p-10 text-center text-gray-600">
                        ...
                        >
                          <div>
                            ...
                            <div className="text-sm text-gray-600">
                              Vai trò:{" "}
                              <span className="font-medium">{iv.role}</span>
                              {iv.project?.key ? (
